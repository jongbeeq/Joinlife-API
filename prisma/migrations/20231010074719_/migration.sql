...
CREATE TABLE `Post` (
    ...
    `totalComment` INTEGER NOT NULL DEFAULT 0,
    `userId` INTEGER NOT NULL,
    `linkScheduleId` INTEGER NOT NULL,
...
-- AddForeignKey
ALTER TABLE `UserInterest` ADD CONSTRAINT `UserInterest_interestId_fkey` FOREIGN KEY (`interestId`) REFERENCES `Category`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `EventSchedule` ADD CONSTRAINT `EventSchedule_postDetailScheduleId_fkey` FOREIGN KEY (`postDetailScheduleId`) REFERENCES `Post`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `StatusReportPost` ADD CONSTRAINT `StatusReportPost_postId_fkey` FOREIGN KEY (`postId`) REFERENCES `Post`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
